...
/** outer circle: how far each role may move around its anchor */
export const ORBIT: Record<Role, number> = { def: 88, mid: 110, att: 132 };
...
export function formationSlotsWithRoles(f: Formation, team: Team): Slot[] {
  ...
  f.rows.forEach((count, i) => {
    ...
    for (let j = 0; j < count; j++) {
      const y = PITCH_CY + (j - (count - 1) / 2) * gap;
      slots.push({ x, y, role });
    }
  });
  return slots;
