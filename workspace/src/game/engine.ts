...
export type Mode =
  ...
  | "plan" | "resolve" | "goal" | "roundEnd" | "champion" | "howto";
...
export class Engine {
  ...
  private lockPlan() {
    ...
    if (this.net.active) {
      if (this.net.isHost) {
        ...
        if (this.net.guestPlanIn) this.startResolve();
      } else if (!this.net.planSent) {
        // guest ships their secret orders to the host
        this.net.send({ t: "plan", p: this.plans[this.net.myTeam] });
        this.net.send({ t: "locked" });
  ...
  private update(dt: number) {
    ...
    if (
      ...
      this.net.hostLocked && this.net.guestPlanIn
    ) {
      this.net.guestPlanIn = false;
      this.net.oppLocked = false;
      this.startResolve();
  ...
  private onDown = (e: PointerEvent) => {
    ...
    if (gi == null) return;
    // the carrier is frozen — it can only pass
    if (this.ball.mode === "held" && this.ball.holderId === gi) {
      sfx.denied();
      const c = this.coins[gi];
      this.addP({ x: c.x, y: c.y - 36, kind: "text", label: "MUST PASS!", color: "#ffd23f", life: 0.8, vy: -36, size: 16 });
      return;
    }
  ...
  private onMove = (e: PointerEvent) => {
    ...
    if (this.mode === "plan" && !this.drag) {
      ...
      const locked = this.hoverCoin != null && this.ball.mode === "held" && this.ball.holderId === this.hoverCoin;
      this.cv.style.cursor = nearBall || (this.hoverCoin != null && !locked) ? "grab" : this.hoverCoin != null ? "not-allowed" : "default";
    }
  ...
  private drawOrbits(c: CanvasRenderingContext2D) {
    ...
    this.coins.forEach((cn, gi) => {
      ...
      if (gi === lockedGi) {
        ...
        c.fillStyle = "#ffd23f";
        c.fillText("LOCKED · DRAG THE BALL ▸", cn.ax, cn.ay + R_COIN + 17);
        return;
      }
  ...
  private drawPlans(c: CanvasRenderingContext2D) {
    ...
    this.coins.forEach((cn, gi) => {
      ...
      if (mv) {
        ...
        c.closePath(); c.fill();
        c.globalAlpha = 0.4;
        const em = makeEmblem(this.flags[team], 46, col);
        c.drawImage(em, mv.x - 23, mv.y - 23, 46, 46);
        c.globalAlpha = 0.8;
        c.setLineDash([4, 5]);
        c.lineWidth = 1.6;
  ...
  private drawCoins(c: CanvasRenderingContext2D) {
    this.coins.forEach((cn, gi) => {
      ...
      // face emblem
      const em = makeEmblem(this.flags[cn.team], 40, "rgba(0,0,0,0.35)");
      c.save();
      c.beginPath(); c.arc(x, y, R_COIN - 4, 0, 7); c.clip();
      c.drawImage(em, x - 20, y - 20, 40, 40);
      c.restore();
  ...
  private drawBall(c: CanvasRenderingContext2D) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const px = Math.cos(a) * 4.6, py = Math.sin(a) * 4.6;
    ...
    for (let i = 0; i < 5; i++) {
      ...
      c.beginPath();
      c.arc(Math.cos(a) * 10.4, Math.sin(a) * 10.4, 3.4, 0, 7);
      c.fill();
    }
