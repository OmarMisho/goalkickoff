...
export class Engine {
  ...
  private startResolve() {
    ...
    // ONLINE host: send the FULL reel (no compaction) so both devices
    // have identical keyframes — zero interpolation drift
    if (this.net.active && this.net.isHost) {
      this.net.send({
        t: "resolve",
        p: {
          ck: R.coinKeys.map((arr) => arr.map((k) => ({ t: +k.t.toFixed(3), x: k.x, y: k.y }))),
          bk: R.ballKeys.map((k) => ({ t: +k.t.toFixed(3), x: k.x, y: k.y })),
          events: R.events,
          fw: R.flightWindow,
          result: R.result,
          scores: this.scores,
