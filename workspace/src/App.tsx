...
import { FORMATIONS, TEAM_COLOR, TICKS_PER_ROUND, SEG_SECS, WIN_GOALS } from "./game/core";
  ...
  return (
    <div className="stage-bg w-full h-full overflow-hidden select-none">
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        ...
        {/* planning panel */}
        {ui.mode === "plan" && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 md:gap-4 flex-wrap justify-center px-2">
            <div className="panel chamfer px-4 py-2">
              <div className="font-disp text-sm" style={{ color: pcol }}>
                {ui.online ? `YOUR ORDERS · P${(ui.myTeam ?? 0) + 1}` : `PLAYER ${ui.planner + 1} — SECRET ORDERS`}
              </div>
              <div className="text-[10px] tracking-[0.2em] text-white/50 font-bold mt-0.5">
                SEGMENT {Math.min(ui.tick + 1, TICKS_PER_ROUND)}/{TICKS_PER_ROUND}
                {ui.online && ui.oppLocked && <span className="text-[#3fd46d] ml-2">RIVAL LOCKED ✓</span>}
                {ui.online && ui.awaiting && <span className="text-[#ffd23f] ml-2 flight-flash">SENT — SYNCING…</span>}
              </div>
            </div>
            {ui.online && (ui.awaiting || ui.lockedIn) ? (
              <div className="flex flex-col items-center justify-center px-4 py-2 panel chamfer-sm">
                <span className="font-disp text-lg text-[#ffd23f] flight-flash">● ● ●</span>
                <span className="text-[9px] tracking-[0.16em] text-white/55 font-bold">
                  {ui.oppLocked
                    ? "CLASHING…"
                    : ui.myTeam === 1
                      ? "WAITING FOR CLASH"
              ...
              <TimerRing left={ui.planLeft} total={ui.planTotal} />
            )}
            <div className="flex flex-col gap-1.5">
              <button onClick={() => eng().uiLock()} disabled={ui.awaiting}
                className="btn-blade font-disp text-base px-7 py-2 text-[#04121c] disabled:opacity-40" style={{ background: "#ffd23f" }}>
                {ui.awaiting ? "SENT ✓" : "LOCK IN ▸"}
              </button>
              <button onClick={() => eng().uiClearPlan()} disabled={ui.awaiting}
                className="btn-blade font-disp text-[11px] px-7 py-1.5 text-white/75 bg-[#12384c] disabled:opacity-40">
                CLEAR
              </button>
            </div>
          </div>
...
function RotateHint() {
  return (
    <div className="fixed inset-0 z-50 stage-bg flex flex-col items-center justify-center gap-4 text-center px-6">
      <svg width="64" height="64" viewBox="0 0 24 24" fill="none" className="rotate-wiggle">
      ...
      <div className="font-disp text-2xl text-white" style={{ textShadow: "3px 3px 0 rgba(25,211,255,0.4)" }}>
        TURN YOUR PHONE
      </div>
      <div className="text-sm text-white/70 max-w-[260px] leading-relaxed">
        Kickoff Tactics plays best in <span className="text-[#ffd23f] font-bold">landscape</span> —
        rotate your device to see the whole pitch.
      </div>
    </div>
  );
}
