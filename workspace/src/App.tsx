...
export default function App() {
  ...
  const stopHelloRetry = () => {
    if (helloTimer.current != null) { window.clearInterval(helloTimer.current); helloTimer.current = null; }
  };
  const teardownNet = () => { stopHelloRetry(); netRef.current?.close(); netRef.current = null; };
  ...
  const onPeerMsg = (m: any) => {
    ...
    if (m.t === "welcome") {
      if (e.netActive()) return; // already started — ignore duplicates
      stopHelloRetry();
      setN({ oppJoined: true, oppFlag: m.flag ?? "blaugrana", status: "connected" });
  ...
  const startJoining = (code: string) => {
    ...
    netRef.current = joinRoom(code, {
      onRoom: () => {
        // keep announcing ourselves until the host answers with "welcome"
        const sayHello = () => netRef.current?.send({ t: "hello", flag: myFlagRef.current });
        sayHello();
        stopHelloRetry();
        helloTimer.current = window.setInterval(() => {
          if (!netRef.current || engRef.current?.netActive()) { stopHelloRetry(); return; }
          sayHello();
        }, 1200);
      },
      onPeer: onPeerMsg,
  ...
  return (
    <div className="stage-bg w-full h-full overflow-hidden select-none">
      <div className="relative w-full h-full flex items-center justify-center">
        ...
        {/* resolve caption — at top for portrait, bottom for landscape */}
        {ui.mode === "resolve" && (
          <div className="absolute top-[72px] md:top-auto md:bottom-3 left-1/2 -translate-x-1/2 z-20 pointer-events-none">
            {ui.ballInFlight ? (
              <div
                className="font-disp text-base md:text-2xl tracking-[0.18em] px-4 py-1.5 md:px-6 md:py-2 chamfer-sm flight-flash"
                style={{ background: TEAM_COLOR[ui.flightTeam ?? 0], color: "#04121c" }}
              >
                ⚽ BALL IN FLIGHT — CUT IT!
              </div>
            ) : (
              <div className="font-disp text-xs md:text-sm tracking-[0.3em] text-white/60 px-4 py-1 md:px-5 md:py-1.5 panel chamfer-sm">
                RESOLVE — PLANS CLASH
              </div>
            )}
          </div>
        ...
        {/* planning panel — docks to the top on phones, bottom-center on desktop */}
        {ui.mode === "plan" && (
          <div
            className="absolute top-[72px] left-1/2 -translate-x-1/2 z-20 flex flex-row items-center gap-2 w-[95%] max-w-[500px]
              md:top-auto md:bottom-3 md:w-auto md:max-w-none md:flex-row md:items-center md:gap-4 md:px-2"
          >
            <div className="panel chamfer px-2 py-1 text-center md:text-left md:px-4 md:py-2 flex-1 md:flex-none">
              <div className="font-disp text-[10px] leading-tight md:text-sm" style={{ color: pcol }}>
                {ui.online ? `ORDERS · P${(ui.myTeam ?? 0) + 1}` : `P${ui.planner + 1} SECRET`}
              </div>
              <div className="text-[7px] md:text-[10px] tracking-wider md:tracking-[0.2em] text-white/50 font-bold mt-0.5">
                SEG {Math.min(ui.tick + 1, TICKS_PER_ROUND)}/{TICKS_PER_ROUND}
              </div>
              {ui.online && ui.oppLocked && (
                <div className="text-[7px] md:text-[10px] font-bold text-[#3fd46d] mt-0.5">RIVAL LOCKED ✓</div>
              )}
              {ui.online && ui.awaiting && (
                <div className="text-[7px] md:text-[10px] font-bold text-[#ffd23f] flight-flash mt-0.5">SYNCING…</div>
              )}
            </div>
            {ui.online && (ui.awaiting || ui.lockedIn) ? (
              <div className="flex flex-col items-center justify-center px-2 py-1 md:px-4 md:py-2 panel chamfer-sm">
                <span className="font-disp text-sm md:text-lg text-[#ffd23f] flight-flash">● ● ●</span>
                <span className="text-[7px] md:text-[9px] tracking-[0.12em] md:tracking-[0.16em] text-white/55 font-bold text-center">
                  {ui.oppLocked
                    ? "CLASHING…"
                    : ui.myTeam === 1
                      ? "WAITING"
              ...
              <div className="self-center md:self-auto">
                <TimerRing left={ui.planLeft} total={ui.planTotal} size={64} />
              </div>
            )}
            <div className="flex flex-col gap-1">
              <button onClick={() => eng().uiLock()} disabled={ui.awaiting}
                className="btn-blade font-disp text-xs md:text-base px-3 py-1.5 md:px-7 md:py-2 text-[#04121c] disabled:opacity-40" style={{ background: "#ffd23f" }}>
                {ui.awaiting ? "SENT ✓" : "LOCK ▸"}
              </button>
              <button onClick={() => eng().uiClearPlan()} disabled={ui.awaiting}
                className="btn-blade font-disp text-[10px] md:text-[11px] px-3 py-1 md:px-7 text-white/75 bg-[#12384c] disabled:opacity-40">
                CLEAR
              </button>
            </div>
          </div>
