...
export function NetLobby({
  ...
  const peerSetRef = useRef(false); // have we received the rival's latest 'set'?
  myRef.current = my;
  ...
  const tryStart = () => {
    ...
    // Only kick off once we hold a confirmed copy of the rival's latest
    // formation + emblem — otherwise the two phones would paint different
    // sticks/ball.
    if (!peerSetRef.current) return;
  ...
  const handleMsg = (m: NetMsg) => {
    switch (m.k) {
      ...
      case 'set':
        ...
        peerSetRef.current = true; // rival's latest emblem/formation confirmed
        break;
      case 'ready':
        updatePeer({ ready: m.ready });
        // Re-broadcast our own latest formation/emblem so the rival always
        // kicks off with an accurate picture, then give the sync a beat to land.
        sendMySet(bridgeRef.current!);
  ...
  const toggleReady = () => {
    ...
    setMyReady(v);
    sfx.select();
    // Ship our latest formation/emblem first so the host never starts cold.
    sendMySet(b);
    b.send({ k: 'ready', ready: v });
    if (v) window.setTimeout(tryStart, 120);
  };
