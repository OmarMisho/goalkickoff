...
export function Scoreboard({ hud, settings }: { hud: HudState; settings: Settings }) {
  ...
  const plate = (team: 0 | 1) => {
    ...
    return (
      <div
        className={`flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-1 sm:py-1.5 bg-[#06251a] border-2 transition-all duration-200 ${
          team === 0 ? 'skew-x-[-8deg] border-l-[4px] sm:border-l-[6px]' : 'skew-x-[8deg] border-r-[4px] sm:border-r-[6px]'
        }`}
        style={{
      ...
      >
        <div className="skew-x-[8deg] flex flex-col items-start min-w-[64px] sm:min-w-[92px]" style={team === 1 ? { transform: 'skewX(-8deg)' } : undefined}>
          <div className="font-body font-bold text-[9px] sm:text-[11px] tracking-[0.18em] leading-tight max-w-[80px] sm:max-w-[120px] truncate" style={{ color: t.light }}>
            {name}
          </div>
          <div className="font-body text-[8px] sm:text-[10px] font-semibold tracking-widest text-[#5f8f77]">{fm.name} • {fm.tag}</div>
        ...
        <div
          className="font-display text-2xl sm:text-4xl leading-none tabular-nums"
  ...
  return (
    <div className="pointer-events-none flex items-stretch gap-1 sm:gap-2 z-20 max-w-[calc(100vw-80px)]">
      {plate(0)}
      <div className="skew-x-[-8deg] flex flex-col items-center justify-center px-2 sm:px-4 bg-[#04150d] border-2 border-[#1c5c40] shadow-[4px_4px_0_rgba(0,0,0,0.5)]">
        <div className="font-body text-[8px] sm:text-[9px] font-bold tracking-[0.3em] text-[#5f8f77]">FIRST TO {TARGET_SCORE}</div>
        <div
          className={`font-display text-lg sm:text-2xl leading-none tabular-nums ${urgent ? 'animate-pulse' : ''}`}
          style={{ color: urgent ? '#ff4d1c' : '#eaffdf' }}
        >
          {hud.phase === 'aim' ? hud.clock : '—'}
        </div>
        <div className="font-body text-[8px] sm:text-[10px] font-bold tracking-[0.14em] whitespace-nowrap" style={{ color: st.color }}>
          {st.text}
        </div>
      </div>
      {plate(1)}
